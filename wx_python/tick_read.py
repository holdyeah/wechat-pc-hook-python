...
def tick_read():
    print('检查微信read.txt是否有数据,有就写入到web爆料中去')
    #微信read.txt目录
    ...
    webBaoLiao.run_results_number(PiPei_data,shuju_data.old_PiPei_number,PiPei_number,shuju_data.piPei_info)
    #查询anti是否有新内容,有就加入到anti_info列表中去
    webBaoLiao.run_results_number(anti_data,shuju_data.old_anti_number,anti_number,shuju_data.anti_info)